/****************************************************************************************************************************
  ...
  configuration/reconfiguration and autoconnect/autoreconnect of WiFi and other services without
 Hardcoding.
  ...
  1.3.0   K Hoang      12/04/2021  Fix invalid "blank" Config Data treated as Valid. Fix EEPROM_SIZE
 bug 1.4.0   K Hoang      21/04/2021  Add support to new ESP32-C3 using SPIFFS or EEPROM 1.5.0
 Michael H    24/04/2021  Enable scan of WiFi networks for selection in Configuration Portal 1.5.1
 K Hoang      10/10/2021  Update `platform.ini` and `library.json` 1.6.0   K Hoang      26/11/2021
 Auto detect ESP32 core and use either built-in LittleFS or LITTLEFS library. Fix bug. 1.7.0   K
 Hoang      09/01/2022  Fix the blocking issue in loop() with configurable WIFI_RECON_INTERVAL 1.8.0
 K Hoang      10/02/2022  Add support to new ESP32-S3 1.8.1   K Hoang      11/02/2022  Add LittleFS
 support to ESP32-C3. Use core LittleFS instead of Lorol's LITTLEFS for v2.0.0+ 1.8.2   K Hoang
 21/02/2022  Optional Board_Name in Menu. Optimize code by using passing by reference 1.9.0   K
 Hoang      09/09/2022  Fix ESP32 chipID and add getChipOUI() 1.9.1   K Hoang      28/12/2022  Add
 Captive Portal using AsyncDNSServer 1.10.1  K Hoang      12/01/2023  Added public methods to load
 and save dynamic data. Bump up to v1.10.1 1.10.2  K Hoang      15/01/2023  Add Config Portal
 scaling support to mobile devices 1.10.3  K Hoang      19/01/2023  Fix compiler error if EEPROM is
 used 1.10.5  K Hoang      29/01/2023  Using PROGMEM for strings. Sync with ESP_WiFiManager_Lite
 v1.10.5
 *****************************************************************************************************************************/
...
#      endif
#    else
// For core v1.0.6-, if not specified any, use SPIFFS to not forcing user to install LITTLEFS
// library
#      if !(defined(USE_LITTLEFS) || defined(USE_SPIFFS))
#        define USE_SPIFFS true
#      endif
...
// #if ( ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(2, 0, 0) )  //(ESP_ARDUINO_VERSION_MAJOR >=
// 2)
#      if (defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 2))
...
// Otherwise, default values (MRD_TIMES = 3, MRD_TIMEOUT = 10 seconds and MRD_ADDRESS = 0) will be
// used Number of subsequent resets during MRD_TIMEOUT to activate
#    ifndef MRD_TIMES
#      define MRD_TIMES 3
#    endif
...
const char ESP_WM_LITE_HTML_HEAD_START[] PROGMEM
    = "<!DOCTYPE html><html><head><title>ESP_ASYNC_WM_LITE</title><meta name='viewport' "
      "content='width=device-width, initial-scale=1'>";

const char ESP_WM_LITE_HTML_HEAD_STYLE[] PROGMEM
    = "<style>div,input{padding:5px;font-size:1em;}input{width:95%;}body{text-align: "
      "center;}button{background-color:#16A1E7;color:#fff;line-height:2.4rem;font-size:1.2rem;"
      "width:100%;}fieldset{border-radius:0.3rem;margin:0px;}</style>";
...
const char ESP_WM_LITE_HTML_SCRIPT_ITEM[] PROGMEM
    = "udVal('{d}',document.getElementById('{d}').value);";
...
#  endif
    ...
    if (hadConfigData && noConfigPortal && (!isForcedConfigPortal)) {
      ...
      ESP_WML_LOGDEBUG(noConfigPortal ? F("bg: noConfigPortal = true")
                                      : F("bg: noConfigPortal = false"));

      for (uint16_t i = 0; i < NUM_WIFI_CREDENTIALS; i++) {
        if (strlen(ESP_WM_LITE_config.WiFi_Creds[i].wifi_pw) >= PASSWORD_MIN_LEN) {
          ESP_WML_LOGDEBUG5(F("bg: addAP : index="), i, F(", SSID="),
                            ESP_WM_LITE_config.WiFi_Creds[i].wifi_ssid, F(", PWD="),
                            ESP_WM_LITE_config.WiFi_Creds[i].wifi_pw);
          wifiMulti.addAP(ESP_WM_LITE_config.WiFi_Creds[i].wifi_ssid,
                          ESP_WM_LITE_config.WiFi_Creds[i].wifi_pw);
        } else {
          ESP_WML_LOGWARN3(F("bg: Ignore invalid WiFi PWD : index="), i, F(", PWD="),
                           ESP_WM_LITE_config.WiFi_Creds[i].wifi_pw);
        }
      }
      ...
      ESP_WML_LOGDEBUG(isForcedConfigPortal ? F("bg: isForcedConfigPortal = true")
                                            : F("bg: isForcedConfigPortal = false"));

      // If not persistent => clear the flag so that after reset. no more CP, even CP not entered
      // and saved
      if (persForcedConfigPortal) {
        ESP_WML_LOGINFO1(F("bg:Stay forever in CP:"),
                         isForcedConfigPortal
                             ? F("Forced-Persistent")
                             : (noConfigPortal ? F("No ConfigDat") : F("DRD/MRD")));
      } else {
        ESP_WML_LOGINFO1(F("bg:Stay forever in CP:"),
                         isForcedConfigPortal
                             ? F("Forced-non-Persistent")
                             : (noConfigPortal ? F("No ConfigDat") : F("DRD/MRD")));
        clearForcedCP();
      }
...
#  if RESET_IF_CONFIG_TIMEOUT
        ...
        // If we're here but still in configuration_mode, permit running TIMES_BEFORE_RESET times
        // before reset hardware to permit user another chance to config.
...
#    else
      ...
      // Still have bug in ESP32_S2 for old core. If using WiFi.setHostname() => WiFi.localIP()
...
#  define MAX_WIFI_CHANNEL 11  // Channel 13 is flaky, because of bad number 13 ;-)
  ...
  String getWiFiSSID(const uint8_t& index) {
    if (index >= NUM_WIFI_CREDENTIALS) return String("");

    if (!hadConfigData) getConfigData();
  ...
  ESP_WM_LITE_Configuration* getFullConfigData(ESP_WM_LITE_Configuration* configData) {
    ...
    if (configData) memcpy(configData, &ESP_WM_LITE_config, sizeof(ESP_WM_LITE_Configuration));
...
#  if SCAN_WIFI_NETWORKS
  int WiFiNetworksFound
      = 0;       // Number of SSIDs found by WiFi scan, including low quality and duplicates
...
#  define RFC952_HOSTNAME_MAXLEN 24
  ...
  char* getRFC952_hostname(const char* iHostname) {
    ...
    size_t len
        = (RFC952_HOSTNAME_MAXLEN < strlen(iHostname)) ? RFC952_HOSTNAME_MAXLEN : strlen(iHostname);
  ...
  void displayConfigData(const ESP_WM_LITE_Configuration& configData) {
    ...
    ESP_WML_LOGERROR3(F("SSID1="), configData.WiFi_Creds[1].wifi_ssid, F(",PW1="),
                      configData.WiFi_Creds[1].wifi_pw);
...
#  define WM_NO_CONFIG "blank"
  ...
  int calcChecksum() {
    ...
    for (uint16_t index = 0;
         index < (sizeof(ESP_WM_LITE_config) - sizeof(ESP_WM_LITE_config.checkSum)); index++) {
...
#  if REQUIRE_ONE_SET_SSID_PW
    ...
    if (!(((strncmp(ESP_WM_LITE_config.WiFi_Creds[0].wifi_ssid, WM_NO_CONFIG, strlen(WM_NO_CONFIG))
          ...
          || ((strncmp(ESP_WM_LITE_config.WiFi_Creds[1].wifi_ssid, WM_NO_CONFIG,
                       strlen(WM_NO_CONFIG))
...
#    define CONFIG_PORTAL_FILENAME_BACKUP ("/wm_cp.bak")
  ...
  void setForcedCP(const bool& isPersistent) {
    uint32_t readForcedConfigPortalFlag
        = isPersistent ? FORCED_PERS_CONFIG_PORTAL_FLAG_DATA : FORCED_CONFIG_PORTAL_FLAG_DATA;
  ...
  bool isForcedCP() {
    ...
    // Return true if forced CP (0xDEADBEEF read at offset EPROM_START + DRD_FLAG_DATA_SIZE +
    // CONFIG_DATA_SIZE)
...
#    if USE_DYNAMIC_PARAMETERS
  ...
  bool checkDynamicData() {
    ...
    for (uint16_t i = 0; i < NUM_MENU_ITEMS; i++) {
      if (myMenuItems[i].maxlen > maxBufferLength) maxBufferLength = myMenuItems[i].maxlen;
    }
    ...
    if (maxBufferLength > 0) {
      ...
      ESP_WML_LOGINFO3(F("CrCCsum=0x"), String(checkSum, HEX), F(",CrRCsum=0x"),
                       String(readCheckSum, HEX));
...
#    endif
  ...
  // Return false if init new EEPROM or SPIFFS. No more need trying to connect. Go directly to
  // config mode
  bool getConfigData() {
...
#    endif
    {
      ...
      ESP_WML_LOGINFO3(F("CCSum=0x"), String(calChecksum, HEX), F(",RCSum=0x"),
                       String(ESP_WM_LITE_config.checkSum, HEX));
...
#    endif
    ...
    if ((strncmp(ESP_WM_LITE_config.header, ESP_WM_LITE_BOARD_TYPE, strlen(ESP_WM_LITE_BOARD_TYPE))
         != 0)
...
#    else
#      if (EEPROM_START + FLAG_DATA_SIZE + CONFIG_DATA_SIZE + FORCED_CONFIG_PORTAL_FLAG_DATA_SIZE \
           > EEPROM_SIZE)
...
#      define BUFFER_LEN 128
    ...
    uint16_t offset
        = CONFIG_EEPROM_START + sizeof(ESP_WM_LITE_config) + FORCED_CONFIG_PORTAL_FLAG_DATA_SIZE;
    ...
    ESP_WML_LOGINFO3(F("ChkCrR:CrCCsum=0x"), String(checkSum, HEX), F(",CrRCsum=0x"),
                     String(readCheckSum, HEX));
...
#  if USING_CUSTOMS_HEAD_ELEMENT
    ...
    if (_CustomsHeadElement) root_html_template += _CustomsHeadElement;
...
#  if SCAN_WIFI_NETWORKS
    ...
    // Replace HTML <input...> with <select...>, based on WiFi network scan in
    // startConfigurationMode()

    ListOfSSIDs = "";

    for (int i = 0, list_items = 0; (i < WiFiNetworksFound) && (list_items < MAX_SSID_IN_LIST);
         i++) {
      if (indices[i] == -1)
      ...
      ListOfSSIDs += String(FPSTR(ESP_WM_LITE_OPTION_START)) + String(WiFi.SSID(indices[i]))
                     + String(FPSTR(ESP_WM_LITE_OPTION_END));
    ...
    if (ListOfSSIDs == "")  // No SSID found or none was good enough
      ListOfSSIDs = String(FPSTR(ESP_WM_LITE_OPTION_START))
                    + String(FPSTR(ESP_WM_LITE_NO_NETWORKS_FOUND))
                    + String(FPSTR(ESP_WM_LITE_OPTION_END));

    pitem = String(FPSTR(ESP_WM_LITE_HTML_HEAD_END));

#    if MANUAL_SSID_INPUT_ALLOWED
    pitem.replace("[[input_id]]", "<input id='id' list='SSIDs'>"
                                      + String(FPSTR(ESP_WM_LITE_DATALIST_START)) + "'SSIDs'>"
                                      + ListOfSSIDs + FPSTR(ESP_WM_LITE_DATALIST_END));
    ESP_WML_LOGDEBUG1(F("pitem:"), pitem);
    pitem.replace("[[input_id1]]", "<input id='id1' list='SSIDs'>"
                                       + String(FPSTR(ESP_WM_LITE_DATALIST_START)) + "'SSIDs'>"
                                       + ListOfSSIDs + FPSTR(ESP_WM_LITE_DATALIST_END));
    ESP_WML_LOGDEBUG1(F("pitem:"), pitem);
#    else
    pitem.replace("[[input_id]]", "<select id='id'>" + ListOfSSIDs + FPSTR(ESP_WM_LITE_SELECT_END));
    pitem.replace("[[input_id1]]",
                  "<select id='id1'>" + ListOfSSIDs + FPSTR(ESP_WM_LITE_SELECT_END));
#    endif
...
#  endif
    ...
    root_html_template += String(FPSTR(ESP_WM_LITE_FLDSET_END)) + FPSTR(ESP_WM_LITE_HTML_BUTTON)
                          + FPSTR(ESP_WM_LITE_HTML_SCRIPT);
...
#  else
        ...
        AsyncWebServerResponse* response
            = request->beginResponse(200, FPSTR(WM_HTTP_HEAD_TEXT_HTML), result);
...
#  endif
      ...
      if (!id_Updated && (key == String("id"))) {
        ...
        if (strlen(value.c_str()) < sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw) - 1)
          ...
          strncpy(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw, value.c_str(),
                  sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw) - 1);
        ...
        if (strlen(value.c_str()) < sizeof(ESP_WM_LITE_config.WiFi_Creds[1].wifi_pw) - 1)
          ...
          strncpy(ESP_WM_LITE_config.WiFi_Creds[1].wifi_pw, value.c_str(),
                  sizeof(ESP_WM_LITE_config.WiFi_Creds[1].wifi_pw) - 1);
      }
#  if USING_BOARD_NAME
        ...
        if (strlen(value.c_str()) < sizeof(ESP_WM_LITE_config.board_name) - 1)
          ...
          strncpy(ESP_WM_LITE_config.board_name, value.c_str(),
                  sizeof(ESP_WM_LITE_config.board_name) - 1);
      }
#  endif
...
#  endif
        ...
        if (isForcedConfigPortal) clearForcedCP();
...
#  endif
    ...
    delay(
        100);  // ref: https://github.com/espressif/arduino-esp32/issues/985#issuecomment-359157428
    ...
    if (server && dnsServer) {
      // CaptivePortal
      // if DNSServer is started with "*" for domain name, it will reply with provided IP to all DNS
      // requests
...
#  if SCAN_WIFI_NETWORKS
  ...
  // https://github.com/khoih-prog/ESP_WiFiManager/blob/master/src/ESP_WiFiManager-Impl.h
  ...
  int scanWifiNetworks(int** indicesptr) {
    ...
    // KH, Terrible bug here. WiFi.scanNetworks() returns n < 0 => malloc( negative == very big ) =>
    // crash!!! In .../esp32/libraries/WiFi/src/WiFiType.h #define WIFI_SCAN_RUNNING   (-1) #define
    // WIFI_SCAN_FAILED    (-2) if (n == 0)
    if (n <= 0) {
